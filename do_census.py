...
from libs.DataHandler import Census
...
# Configuration
this_parse = ArgumentParser(description="Train ARGUE on Census")
add_standard_arguments(this_parse)
this_args = this_parse.parse_args()

experiment_config = [
    Census(
        random_state=this_args.random_seed, y_normal=["Male", "Female"], y_anomalous=["Anomalous"],
        n_train_anomalies=this_args.n_train_anomalies, p_pollution=this_args.p_contamination
    )
]

DIM_TARGET = (600, 300, 150, 75, 30, 15)
DIM_ALARM = ALARM_BIG
BATCH_SIZE = 512
A4RGUE_CLUSTERS = 4
...
if __name__ == '__main__':
    ...
    this_experiment = ExperimentWrapper(
        save_prefix="Census", data_setup=experiment_config,
