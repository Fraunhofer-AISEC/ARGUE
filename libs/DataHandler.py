...
class DataLabels:
    ...
    def to_data(
        ...
        :param for_experts: split the target data (one class for each expert) - useful for ARGUE, not for the baselines
        :param equal_size: equalise the amount of samples for each expert by oversampling, i.e. we repeat samples in the smaller sets
        """
...
class Darknet(DataLabels):
    def __init__(
        ...
        # We divide the normal labels (Non-Tor & Non-VPN) in protocol types
        y_train[y_train.isin(["Non-Tor", "NonVPN"])] = train_data.loc[y_train.isin(["Non-Tor", "NonVPN"]), "Protocol"].astype("str")
...
class IDS(DataLabels):
    def __init__(
        ...
        df["Label"] = df["Label"].replace(self._attack_map())
        labels = df.pop("Label")

        # Drop the non-informtive object features
    ...
    def csv_to_h5(self, data_path: Path):
        ...
        for i_file, cur_file in enumerate(all_files):
            ...
            # Divide the benign class into multiple days
            # We could have used cur_df.index.day here, but there are some weird days in the files, which should not be
            # in there if I've read the description correctly - so let's use the file number instead
            cur_df["day"] = i_file
            # Add this attribute to the data so that the other methods can learn it

            cur_df.loc[cur_df["Label"] == "Benign", "Label"] = cur_df.loc[cur_df["Label"] == "Benign", "day"].astype("str")
